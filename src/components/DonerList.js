...
export const DonerList = ({ isEnabled, toggleEnable }) => {
    ...
    try{
        const response=await axios.get("https://localhost:44390/api/DonerPerson/Donerlist")
    ...
    const handleRead = (id) => {
        const selectedDoner = Doner.find(DNR => DNR.ID === id);
        const checkoutUrl = `/DonerRead/${id}`; 
        navigate(checkoutUrl,{ state: { DonerData: selectedDoner } });
        console.log(`Doner  ID: ${id}`);
    };
...
subHeaderComponent={
	...
	/> {showDetails && <DonerRead studentId={selectedStudentId} />}
            <div className='we_six'>
                          ...
                          <div><button onClick={handleContact}>FeedBack</button></div>
                      </div>
              <Footer/>
	</div>
	)
